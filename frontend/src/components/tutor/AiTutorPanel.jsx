...
export default function AiTutorPanel({ visualizationId, vizData, provider, model, onRetract }) {
  ...
  return (
    <section className="panel space-y-5 rounded-md p-5">
      ...
      <div className="grid gap-5 bg-paper p-4 md:grid-cols-[180px_1fr] md:items-center">
        <AvatarAnimator isSpeaking={player.isPlaying} name="Alex" />
        <div>
          <CaptionBar text={player.currentCaption || 'Press Start to begin the AI Tutor explanation'} isVisible />
          <p className="mt-2 text-center text-xs font-semibold text-ink/45">Total duration ~{Math.max(1, Math.ceil(tutorScript.total_estimated_seconds / 60))} min</p>
        </div>
      </div>
