...
export default function ApiKeyInput({ provider, keyHint, onSaved }) {
  ...
  return (
    <div className="space-y-2">
      <label className="block text-sm font-medium">Provider API key</label>
      <div className="flex gap-2">
        <div className="relative flex-1">
          <KeyRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink/45" />
          <input
            className="focus-ring w-full border border-black/15 bg-white py-2 pl-9 pr-3"
        ...
        <button className="focus-ring inline-flex items-center gap-2 bg-ink px-4 py-2 text-sm font-semibold text-white" onClick={save} type="button">
